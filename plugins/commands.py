...
async def start(client, message):
    ...
    if len(message.command) != 2:
        await message.reply_photo(
            ...
            caption= """Hᴇʟʟᴏ Dᴀʀʟɪɴɢ, Tʜɪs RᴏBᴏᴛ ᴄᴀɴ ᴘʀᴏᴠɪᴅᴇ ʟᴀᴛᴇsᴛ Mᴏᴠɪᴇs/Sᴇʀɪᴇs/Aɴɪᴍᴇs ɪɴ <u>ᴛᴇʟᴜɢᴜ + Mᴜʟᴛɪ</u> ʙʏ ᴊᴜsᴛ ᴛʏᴘɪɴɢ ᴛʜᴇ ᴛɪᴛʟᴇ 😇""",
       ...
       # ]]       
        ...
        #reply_markup = InlineKeyboardMarkup(InlineKeyboardButton('✇ Jᴏɪɴ Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ ✇', url=CHNL_LNK))
    ...
    if not files_:
        ...
        try:
            ...
            title = '@File_Search_RoBot  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@File_Search_RoBot  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            subscription_text = "💳 Get free subscription 💳"
            callback_data = "https://t.me/AniMovieRulz/23"
